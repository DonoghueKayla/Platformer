...
var actorChars = {
  "@": Player,
  "a": Diamond,
  "0": Coin, // A coin will wobble up and down
  "=": Goomba
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  else if(type == "tramp" && this.status == null)	
  {
	  this.status = "trampoline";
	  
  }  
  else if (type == "winWall")
  {	  this.status = "won";
	  this.finishDelay = 1;}  
...
function runGame(plans, Display) {
  function startLevel(n) {
    ...
    runLevel(new Level(plans[n]), Display, function(status) {
      ...
      else if (status == "won" && n < plans.length - 1)
        startLevel(n + 1);
      else
        console.log("You win!");
    });
